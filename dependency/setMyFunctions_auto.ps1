...
if(Test-Path $DEPENDENCY.TheConfigurationFile) 
{
    ...
    if(Test-Path $DIRECTORY.myfunctions)
    {

        #Write-Output "<-------------------{ Loading Dependencies }-------------------------->"
        Write-Host "[ INCLUDE-ALL ] The {*.ps1} from the $($DIRECTORY.myfunctions)" -ForegroundColor Cyan
    
        # Get all the *.ps1 files in the "myfunctions" directory
        $myfunctions_files = Get-ChildItem -Path $DIRECTORY.myfunctions -Filter "*.ps1" | Select-Object -ExpandProperty FullName
    
        Write-Debug "Directory myfunctions : $($DIRECTORY.myfunctions)"
        # Loop through each myfunctions file and include it in the profile
        foreach ($path in $myfunctions_files) {
            if (Test-Path $path) {
        ...
        Write-Output "[ OK ] All Profile Functions => { setAllmyfunctionss.ps1 } Loaded Successfully"
    } else { Write-Error "myfunctions directory path is empty : $($DIRECTORY.myfunctions)"} 
