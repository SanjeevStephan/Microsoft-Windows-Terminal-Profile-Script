...
if(Test-Path $DEPENDENCY.TheConfigurationFile) 
{
    ...
    if(Test-Path $DIRECTORY.remoteScript)
    {

        #Write-Output "<-------------------{ Loading Dependencies }-------------------------->"
        Write-Host "[ INCLUDE-ALL ] The {*.ps1} from the $($DIRECTORY.remoteScript)" -ForegroundColor Cyan
    
        # Get all the *.ps1 files in the "remoteScript" directory
        $remoteScript_files = Get-ChildItem -Path $DIRECTORY.remoteScript -Filter "*.ps1" | Select-Object -ExpandProperty FullName
    
        Write-Debug "Directory remoteScript : $($DIRECTORY.remoteScript)"
        # Loop through each remoteScript file and include it in the profile
        foreach ($path in $remoteScript_files) {
            if (Test-Path $path) {
    

                Write-Host "[ OK ] Included : remoteScript Functions => { $(Split-Path -Leaf $path) } Successfully" -ForegroundColor  Green
        ...
        Write-Output "[ OK ] All Profile Functions => { setAllremoteScripts.ps1 } Loaded Successfully"
    } else { Write-Error "remoteScript directory path is empty : $($DIRECTORY.remoteScript)"} 
