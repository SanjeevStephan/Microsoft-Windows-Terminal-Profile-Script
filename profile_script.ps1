...
.DESCRIPTION
    ...
    if it is available and enabled then it checks if the key in the 'profile_config.ps1' has a matching '$($script.Name)' in the '$script_json'
...
[
    {
        "Name" : "test",
        "File" : "test.ps1",
        "Path" : "C:\\Users\\Sanju\\Documents\\PowerShell\\test.ps1",
        "Desc" : "Simple Test Script"
    }
...
# Convert the JSON into HashTable
$script_db = ConvertFrom-Json $script_json 
...
{
    if($Functionality["display_script_info"] -eq "enable")
    {
        $choice = check_config
