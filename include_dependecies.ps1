...
$profile_dependencies = @{
    ...
    "getLink"            = "$myscript_source\getLink.ps1"
    "getScript"          = "$myscript_source\getScript.ps1"
    "setProfileFunctions"= "$myscript_source\setProfileFunctions.ps1"  
...
foreach ($dependency in $profile_dependencies.GetEnumerator() | Sort-Object -Property Key ) {
    ...
    if (Test-Path $path) {
        Write-Host "[ OK ] Including $($dependency.Key) dependency: $path" -ForegroundColor Green
       . $path
    } else {
        Write-Warning "Dependency $($dependency.Key) not found at path $path"
    }
}
