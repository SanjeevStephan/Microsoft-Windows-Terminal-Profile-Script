...
$copy_to_dir = $env:terminal # $env:OneDrive\Documents\PowerShell\\data\\backup\\new_terminal\\
...
<# ========================================== 2. Check  $copy_to_dir exists =============================================#>
    ...
    {  #if the backup-directory exists where we have to copy-to the directory  
     Write-Host "|    |          |        |    |     ├──[ FOUND ] BACKUP : $copy_to_dir" -ForegroundColor Yellow 
<# ========================================== 3. Check  $copy_to_file exists & Renamed to $renamed_file_with_fullpath ==========================#>
        ...
        {   # if file exists at path '/PowerShell/data/backup/terminal_settings/setting.json' 
            ...
            Write-Host "|    |          |        |    |     ├──[ FOUND ] FILE-PATH : $renamed_file_with_fullpath" -ForegroundColor Yellow 
            mv $copy_to_file $renamed_file_with_fullpath
            Write-Host "|    |          |        |    |     ├──[ RENAMED ] $backup_filename -> $renamedFile" -ForegroundColor Cyan
            # and then backup the source file named 'settings.json'.
            #BackupNow
<# ============================== 4. Check $renamed_file_with_fullpath exists & Delete $renamed_file_with_fullpath & BackupNow() $==========================#>
        } elseif(Test-Path $renamed_file_with_fullpath) 
        {   # if file exists at path '/PowerShell/data/backup/terminal_settings/setting.old.json' 
            Write-Host "|    |          |        |    |     ├──[ FOUND ] FILE : $renamed_file_with_fullpath" -ForegroundColor Yellow 
            # this means 'setting.old.json' and 'settings.json' are not allowed to exists at the same time in the same universe directory.
            rm $renamed_file_with_fullpath 
